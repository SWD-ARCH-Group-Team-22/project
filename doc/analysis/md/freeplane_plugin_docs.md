...
## Freeplane SVG Plugin
It embeds SVG support within the software. It allows users to:
- Import SVG images within a node
- Export the mindmap in pdf and svg formats
- Set a SVG image as background  
It exploits the Apache Batik and Apache FOP libraries: the former is in charge of visualizing and rendering SVG images, the latter to export a Freeplane file as PDF.  
It is composed of some crucial components:
1. `SvgViewerFactory`: it builds the SVG image; it does so by calling the Batik APIs, and it render the SVG image asynchronously
2. `BitmapViewerFactory/BitmapViewerComponent`: these components are used for both importing raster images and to serve the swing pipeline for visualizing SVG files (Swing requires them to be raster images).
3. `ExportVectorGraphic / ExportSvg / ExportPdf`: classes that handle exporting faetures. `ExportSvg` and `ExportPdf` both inherit from `ExportVectorGraphic`.

This plugin suffered from bugs during its lifecycle: the main one concerning the Look and Feel (graphic rendering) and its interaction with the tool for exporting images. For more than 10 years bugs were fixed, and finally in 2024 the issue was solved: the LaF doesn't change anymore while exporting.
Another issue was related to the rasterization of the image in the workstation: many commits during the 2010s aimed at fixing exceptions and problems: nowadays, the Freeplane application provides an out-of-bound error, to avoid importing large images, and the zooming tool respects the original image size proportions.

It is also affected by -R command-line instruction, as most of the Freeplane infrastructure
### Communication
The Freeplane SVG plugin depends on the Freeplane core only. It is a sort of wrapper class that acts as a bridge between the core and the APIs from external libraries that handle the real conversion and SVG images visualization
Main dependencies are:
- `ModeController/ MModeController, UITools`: to interact with the workstation to show images
- `CommandLineOptions`: as most of the Freeplane architectural components, it is affected by command-line instructions defined at run-time.
- `MapModel, NodeModel, Controller`: to interact directly with single nodes within a map
- Interfaces to define Viewers for SVG images within the workstation.

## Freeplane AI
From the end of 2025 onwards, Freeplane supports AI and LLMs directly in the Freeplane workstation.  
AI can:
- Understand nodes and their context
- Understand its position within the MindMap
- Edit multiple nodes at the same time
- Delete nodes
- Set tags in a node
- Keep memory of the convestation and its state

It is communicates with other cores inside the Freeplane architecture the same way groovy scripts communicate through API.  
Freeplane actively supports multiple agents, and developers have used Claude Code and other coding assistant to build production code.  
### Communication
The AI plugins communicates with the core for:
- Formatting UI, and getting control of the mindmap workstation (`NodeController, MNodeController, UIUtils`)
- Getting information about command-line instructions (`CommandLineOptions`)
- Getting APIs from user secret files (`ResourceController`)
- Save logs (`LogUtils`)
